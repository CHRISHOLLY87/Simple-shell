...
char *builtin_files[8] =  {
	"./comment.so", "./setprompt.so", "./cd.so", "./bgjobs.so", "./fg.so", "./loadpluggin.so", "./culater.so"
};
...
int execute_built_in(char **argv){
	...
	int i;
	int (*function)(char **);
	for (i = 0; i < sizeof(builtin_functions); i++){//POTENTIAL ERROR WITH SIZEOF
		if (argv[0] == builtin_functions[i]){
